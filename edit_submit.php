<?php  
    ...
    if(!preg_match('/^[0-9]{10}+$/', $mobile)){
     ...
     //    }
        // }
     $file =$_REQUEST['old_pic'];
    
    if(!empty($_FILES['upload']['name'])){
        if(!empty($file)){
            unlink("C:/wamp/www/batchjuly/img/".$file);
    }
    //    die();
        ...
        $file =$strtotime.'_'.$file_name;
       // $time=date("d-m-y")."-".time();
        //$newname = $time."-".$file_name;
        $fileupload =move_uploaded_file($file_tmp,"images/".$file);    
      ...
      if((!@$nerr) && (!@$span1)  && (!@$span2) &&(!@$eerr)&&(!@$merr)&&(!@$span3)&&(!@$aerr)&&(!@$span)&&(!@$span5)&&(!@$span14)){
       ...
       $id = $_REQUEST['id'];
     $update_query =" UPDATE `register` SET `name`='".$new_name."',`email`='".$new_email."',`mobile`='".$new_mobile."',`address`='".$new_address."',`gender`='".$new_gender."',`language`='".$new_language."',`branch`='".$new_branch."' ,`fileupload`='".$file."' WHERE id= '".$id."' ";
   
     $perform = mysqli_query($con,$update_query);


     $_SESSION['message1']="record updated successfully";
